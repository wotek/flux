...
// MemoryCatalogStore provides an in-memory implementation of [Store].
...
func (s *MemoryCatalogStore) Get(ctx context.Context, id string) (ProductView, bool, error) {
	s.mu.RLock()
	defer s.mu.RUnlock()
...
func (s *MemoryCatalogStore) Save(ctx context.Context, view ProductView) error {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *MemoryCatalogStore) List(ctx context.Context) ([]ProductView, error) {
	s.mu.RLock()
	defer s.mu.RUnlock()
