...
// SetPrice is a command instructing the system to set or update the price for a product.
type SetPrice struct {
	ProductID string `json:"product_id"`
	Price     int    `json:"price"` // in cents
}

// SetPriceHandler executes the [SetPrice] command against the pricing aggregate repository.
type SetPriceHandler struct {
	repo *flux.AggregateRepository[*pricing.PricingAggregate, pricing.PricingEvent]
}

// NewSetPriceHandler creates a new [SetPriceHandler].
func NewSetPriceHandler(repo *flux.AggregateRepository[*pricing.PricingAggregate, pricing.PricingEvent]) *SetPriceHandler {
